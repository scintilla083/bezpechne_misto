...
from bot.states import MayorState, MainMenuState
...
@mayor_router.message(MayorState.collecting, F.text == "✅ Завершити збір інформації")
async def finish_collecting_mayor(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        if not appeal.text.strip():
            await message.answer(
                "Будь ласка, спочатку надішліть інформацію про вашу проблему.",
                reply_markup=collecting_keyboard(),
            )
            return

        # Показываем подтверждение (НЕ удаляем сообщения пользователя)
        await state.set_state(MayorState.confirming)
        await send_clean_message(
            bot=bot,
            message=message,
...
@mayor_router.message(MayorState.collecting)
async def collect_mayor(message: Message, state: FSMContext, bot: Bot):
    """Сбор данных для обращения в городской совет"""
...
@mayor_router.callback_query(MayorState.confirming, F.data == "confirm_yes")
async def confirm_mayor_yes(callback: CallbackQuery, state: FSMContext, bot: Bot):
...
@mayor_router.callback_query(MayorState.confirming, F.data == "confirm_no")
async def confirm_mayor_no(callback: CallbackQuery, state: FSMContext, bot: Bot):
